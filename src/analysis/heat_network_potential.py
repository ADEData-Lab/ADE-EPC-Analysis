...
class HeatNetworkPotentialAnalyzer:
    ...
    def calculate_heat_density(
        ...
        if area_col and area_col in df.columns:
            ...
            # Use property density as proxy
            geo_summary['heat_density_proxy'] = (
                geo_summary['total_heat_demand_gwh'] / geo_summary['property_count']
            )
    ...
    def classify_density_tier(self, heat_density: float) -> str:
        """
        Classify heat density into tiers.

        Args:
            heat_density: Heat density (kWh/m/year or proxy metric)
    ...
    def identify_priority_areas(
        ...
        if 'heat_density_gwh_per_km2' in density_df.columns:
            ...
            density_df['heat_density_kwh_per_m'] = density_df[density_col] * 1000
        else:
            density_col = 'heat_density_proxy'
            # Proxy is already in appropriate units
            density_df['heat_density_kwh_per_m'] = density_df[density_col]

        density_df['density_tier'] = density_df['heat_density_kwh_per_m'].apply(
        ...
        priority_areas = priority_areas.sort_values('heat_density_kwh_per_m', ascending=False)
        ...
        if len(priority_areas) > 0:
            ...
            for idx, row in priority_areas.head(10).iterrows():
                logger.info(
                    f"  {row[geography_col]}: {row['density_tier']} "
    ...
    def calculate_network_potential(
        ...
        # Calculate heat density by area
        density_df = self.calculate_heat_density(df, 'local_authority_name')

        # Classify areas
        if 'heat_density_kwh_per_m' in density_df.columns:
            density_df['heat_density_kwh_per_m'] = density_df['heat_density_gwh_per_km2'] * 1000
        else:
            density_df['heat_density_kwh_per_m'] = density_df['heat_density_proxy']
